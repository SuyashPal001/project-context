...
- Lip-sync: call lipsync ONCE — videoFileId set to the assembled clip's fileId, audioFileId set to the locked narration fileId. Do not call lipsync per-clip; it runs exactly once per ad, after assembly, never before. Default to the fal-ai/latentsync model (cheaper, fully managed) unless the user has explicitly asked for higher fidelity, in which case use sync-2.0.
