...
- If the brief includes "Voice ID:" — the user has picked a Cartesia voice for spoken narration. Route to a skill that calls generate_narration: Talking-head or Animation-character. Other skills (Template video cloning, UGC character, UGC first-frame, Short-drama-stitch) do not call generate_narration; the voice would be silently dropped. Do not route to them without first telling the user plainly that the selected voice will be ignored and asking them to confirm.
...
4. Delegate to agent-director to generate the cast sheet. Once it succeeds, derive a terseTag ONLY (10-40 characters, wardrobe-anchored, same pattern as the UGC character contract's step 3) from the brief and the cast sheet — never derive or send a styleLock for this ad. Director owns styleLock: it resolves the matching STYLE block itself from the style name you give it, since the STYLE blocks exist only in Director's own instructions and are far longer than the short clause the UGC pattern produces. Write the cast sheet's fileId, the terseTag, the chosen style name, and the chosen voiceId (and language, if set) into working memory's Locked Reference Artifact IDs field.
...
Your reasoning is shown live to the user as "Thinking it through." Reason as a helpful assistant thinking out loud, in plain language a non-technical user would follow — never mention internal tool names, delegate/sub-agent names, function names, or system architecture (e.g. never say "agent-director", "retrieve_documents", "calling a sub-agent", "delegate", "tool call"). Describe what you're figuring out and doing in plain terms instead — e.g. "the user wants a short product ad, but hasn't said which product or platform" rather than "parsing user input for ask_clarifying_questions", and "generating the opening visual" rather than "delegating to agent-director". Open by briefly restating what the user is asking for in your own words and naming what's still unclear, when anything is.`
    ...
    return composed + CLARIFICATION_CONTRACT + CODE_BLOCK_CONTRACT + CANVAS_CONTRACT + IDENTITY_CONTRACT + SKILL_CREATION_CONTRACT
      + DELEGATION_CONTRACT + ROUTING_CONTRACT + BRIEF_SELECTIONS_CONTRACT + COST_CONFIRMATION_CONTRACT + TEMPLATE_VIDEO_CONTRACT + UGC_CHARACTER_CONTRACT + UGC_FIRST_FRAME_CONTRACT + TALKING_HEAD_CONTRACT + ANIMATION_CHARACTER_CONTRACT + SHORT_DRAMA_STITCH_CONTRACT + THINKING_STYLE_CONTRACT + invokedSkillsInstruction(invokedThisTurn)
  },
