...
describe('hasStrayQuestion', () => {
  it('flags a question mark in assistant text', () => {
    const messages = [
      { role: 'assistant', content: [{ type: 'text', text: 'Which format do you want?' }] },
    ]
  ...
  it('ignores question marks in non-assistant messages', () => {
    const messages = [
      { role: 'user', content: [{ type: 'text', text: 'What do you think?' }] },
      { role: 'assistant', content: [{ type: 'text', text: 'Got it, working on it now.' }] },
    ]
