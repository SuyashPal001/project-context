...
export async function refundStretchClipCharge(
  ...
  try {
    ...
    try {
      ...
      console.error(
        `[credits] UNREFUNDED STRETCH-CLIP CHARGE: tenantId=${tenantId} chargeKey=${chargeKey} amountMicro=${-net} — ` +
    ...
    console.error(`[credits] refundStretchClipCharge failed tenantId=${tenantId} chargeKey=${chargeKey}:`, (err as Error).message)
  }
}
