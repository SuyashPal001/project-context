...
export async function refundOverlayTextCharge(
  ...
  try {
    ...
    try {
      ...
      console.error(
        `[credits] UNREFUNDED OVERLAY-TEXT CHARGE: tenantId=${tenantId} chargeKey=${chargeKey} amountMicro=${-net} — ` +
    ...
    console.error(`[credits] refundOverlayTextCharge failed tenantId=${tenantId} chargeKey=${chargeKey}:`, (err as Error).message)
  }
}
