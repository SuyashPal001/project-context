...
const AGENT_ORCHESTRATOR_BASE = process.env.NEXT_PUBLIC_AGENT_WS_URL?.replace(/^wss?:\/\//, 'https://') ?? "https://agent-orchestrator.projectcontext.co";
...
export async function relayGet<T>(path: string, agentId: string): Promise<T> {
  const res = await fetch(`${AGENT_ORCHESTRATOR_BASE}${path}`, {
