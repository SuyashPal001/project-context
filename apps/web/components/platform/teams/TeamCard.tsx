        ...
        >
            ...
            {members.length > 0 && (
                <div className="mt-3 flex -space-x-2">
                    {visible.map((agent) => (
                        <PersonaAvatar key={agent.id} persona={agent.persona} avatarUrl={agent.avatarUrl} size={28} className="rounded-full ring-2 ring-background" isDefault={agent.isDefault} />
                    ))}
                    {overflow > 0 && (
