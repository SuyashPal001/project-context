...
filesRoutes.post(
  ...
  zValidator('json', z.object({
    ...
    // Required: the size is signed into the upload URL, so a request without one
    // gets no quota enforcement at all. Every caller was migrated first — making
    // this required before that would have broken agent file generation between
    // the Lambda deploy and the orchestrator restart.
