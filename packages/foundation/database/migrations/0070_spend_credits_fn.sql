-- Hand-written migration: Drizzle does not model functions.
-- All mutation of credit_accounts.balance_micro goes through this function; nothing
-- else writes that column. One statement is one implicit transaction, which is what
-- makes it safe over the Supabase transaction pooler (no BEGIN/COMMIT across backends).
