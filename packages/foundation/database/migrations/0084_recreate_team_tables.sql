-- 0055_cloudy_layla_miller created `teams`/`team_members` and is recorded as
-- applied in drizzle.__drizzle_migrations, but the tables were dropped
-- manually on the dev DB outside the migration system, so `drizzle-kit
-- migrate` never reruns 0055 to recreate them. This replays that DDL under a
-- new migration id so a fresh `drizzle-kit migrate` restores them wherever
-- they're missing, idempotently, without touching envs where they already exist.
